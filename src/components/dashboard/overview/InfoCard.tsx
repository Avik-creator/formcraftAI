...
const TitleWithIcon = ({ title, icon: Icon }: { title: string; icon: React.ElementType }) => {
  return (
    <div className="flex items-center justify-between gap-3">
      <CardTitle>
        <h2 className="bg-clip-text text-transparent bg-gradient-to-r from-white to-zinc-400">
          {title}
        </h2>
      </CardTitle>
  ...
  return (
    <Card
      className={cn(
        ...
        'hover:-translate-y-0.5 hover:border-zinc-700/50 transition-all duration-300',
        className,
      )}
    >
      ...
      <CardContent className={cn('mt-auto', contentClassName)}>
        {renderData ? (
          ...
          <h2 className="font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-zinc-400 sm:text-2xl text-xl">
            {data}
          </h2>
        )}
      </CardContent>
...
export const InfoCardSkeleton = ({ className }: { className?: string }) => {
  return (
    <Card
      className={cn(
        'backdrop-blur-sm bg-black/30 border border-zinc-800/50 shadow-lg transition-all duration-300',
        className,
      )}
    >
